--[[
	...
	iRay  | Programming
	Max   | Programming
  Change key system to KeyGuardian | doitenroi.9941
]]
	...
	if (Settings.KeySystem) then
		...
		if typeof(Settings.KeySettings.Key) == "string" then Settings.KeySettings.Key = {Settings.KeySettings.Key} end
		                  local KeyGuardLibrary = loadstring(game:HttpGet("https://cdn.keyguardian.org/library/v1.0.0.lua"))()
		                  	local trueData = "truelol"
		local falseData = "falselol"
                  KeyGuardLibrary.Set({
                      publicToken = Settings.KeySettings.publicToken or "",
                      privateToken = Settings.KeySettings.privateToken or "",
                      trueData = trueData,
                      falseData = falseData
                  })
function checkvalidkey(key)
    return string.lower(Settings.KeySettings.check) == "free" and KeyGuardLibrary.validateDefaultKey(key) == trueData or string.lower(Settings.KeySettings.check) == "premium" and KeyGuardLibrary.validatePremiumKey(key) == trueData or KeyGuardLibrary.validateDefaultKey(key) == trueData or KeyGuardLibrary.validatePremiumKey(key) == trueData
end
		...
		if isfile and isfile(RayfieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
				if checkvalidkey(readfile(RayfieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension)) then
					Passthrough = true
			end
		end
		...
		if not Passthrough then
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				local FoundKey = ''
					if checkvalidkey(KeyMain.Input.InputBox.Text) then
						KeyFound = true
